...
from models.own_graph_class import *
CUDA_bool = False
...
class LangConvLSTMCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, env,lr):
        ...
        self.vis_representation_size = 60 #60
        self.memory_length = 6*(n_count_words+1) #6*(n_count_words+1)
        self.att_repr_size = 11
        self.vis_lang_representation_size = self.vis_representation_size  + self.att_repr_size
        ...
        #self.atten1 = Attention(self.memory_length, batch_first=True)
        ...
        self.atten2 = Attention(self.lang_representation_size, batch_first=True)
        ...
        #input_lang = torch.ones((batch_size, input_size))

        self.mask_attention_single = f.one_hot(torch.tensor(0), num_classes=self.att_repr_size).unsqueeze(dim=0).float()
        self.mask_attention_init = torch.stack([self.mask_attention_single]*self.n_contexts)
        #self.common_repr = nn.Parameter(torch.ones(self.att_repr_size, requires_grad=True))
        self.common_repr = torch.ones(self.att_repr_size, requires_grad=True)

        ############ Learnable Variables
        # Controlled fixed ones
        template_trafo1 = torch.diag_embed(torch.ones(self.att_repr_size)).roll(shifts=1)
        template_trafo1[0,0]=0
        template_trafo1[self.hidden_size-1, 0]=1
        template_trafo1 = template_trafo1
        template_trafo2 = torch.diag_embed(torch.ones(self.att_repr_size))
        template_trafo3 = torch.diag_embed(torch.ones(self.att_repr_size))*3
        # Later only this
        self.trafos = Variable(torch.stack([template_trafo1, template_trafo2]), requires_grad=True)  # Later learnable
        #self.c_scores = torch.ones(self.n_contexts) # Later learnable
        # Functions with learnable parameters
        self.stacked_inputs_size = (n_count_words+1)+self.task_vector_length 
        self.fc_trafo_scores = nn.Linear(self.stacked_inputs_size, self.n_trafos*self.n_contexts) 
        self.fc_context_scores = nn.Linear(self.stacked_inputs_size, self.n_contexts)
        self.fc_lang = nn.Linear(self.att_repr_size, self.att_repr_size)
    ...
    def forward(self, input_, prev_state, input_lang, prev_state_lang, env_task_list):
        ...
        if(CUDA_bool==False):
           ...
           if(prev_state_lang is None):
               ...
               prev_state_lang = (
                  Variable(torch.stack([self.mask_attention_init]*batch_size), requires_grad=False),  #.cuda()
                  Variable(torch.stack([self.mask_attention_init]*batch_size), requires_grad=False)  #.cuda()
               )
        else:
           if prev_state is None:
           ...
           if(prev_state_lang is None):
               state_size = [batch_size, self.memory_length]
               prev_state_lang = (
                   Variable(torch.stack([self.mask_attention_init] * batch_size)).cuda(),  # .cuda()
                   Variable(torch.stack([self.mask_attention_init] * batch_size)).cuda()  # .cuda()
               )
        #################################
        ## Conv-LSTM
        ...
        # #output_lang, _ = self.atten1(output_lang) # skip connect
        ...
        ## ConvAtt
        prev_hidden_lang, prev_cell_lang = prev_state_lang
        mask_attention = prev_hidden_lang
        stacked_inputs_lang = torch.cat((input_lang, env_task_list), 1)
     
        trafo_score = f.softmax(self.fc_trafo_scores(stacked_inputs_lang), dim = 1).reshape((batch_size, self.n_contexts, self.n_trafos))
        c_scores = f.softmax(self.fc_context_scores(stacked_inputs_lang), dim = 1).reshape((batch_size, self.n_contexts))
        #trafo_score = torch.tensor([[1., 0.],
        #        [0., 1.],
        #        [1., 1.]]).unsqueeze(0)
        trafos_batch = torch.stack([self.trafos]*batch_size)
        common_repr_batch = torch.stack([self.common_repr]*batch_size)
        self.lang_repr = torch.zeros(batch_size, self.att_repr_size)
        for c in range(self.n_contexts):
        #for c in range(1):
          att_trafo = torch.zeros(batch_size, self.att_repr_size, self.att_repr_size)
          #for t in range(1):
          for t in range(self.n_trafos):
            att_trafo_t = torch.mul(trafo_score[:, c, t].unsqueeze(1).unsqueeze(1),trafos_batch[:, t])
            att_trafo = att_trafo + att_trafo_t
          mask_attention_clone = mask_attention.clone()
          mask_attention_clone[:,c] = torch.bmm(mask_attention[:, c], att_trafo)
          c_scores_batch = c_scores[:, c].unsqueeze(1)
          repr_c = mask_attention[:,c].squeeze(1)*common_repr_batch*c_scores_batch
          self.lang_repr = self.lang_repr + repr_c

        #lang_repr = torch.cat((self.lang_repr, env_task_list), 1)
        lang_repr = self.lang_repr
        output_lang = torch.tanh(self.fc_lang(lang_repr))
        hidden_lang = mask_attention_clone
        ...
        ######################################
        ## Motor action
        IsDoMotorAction = torch.sigmoid(self.fc6(stacked_lang_vis_repr))        
        output = torch.sigmoid(self.fc2(stacked_lang_vis_repr))        
        ...
        output_lang = torch.sigmoid(self.fc4_lang(output_lang_repr))
        output_isCount = torch.sigmoid(self.fc5_lang(output_lang_repr))        
        ...
        ### !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        #######!!!!! attention here prev_cell_lang instead of cell lang is passed for experimental reasons!!!
        return [hidden, cell], stacked_output, [hidden_lang, prev_cell_lang], stacked_output_lang
